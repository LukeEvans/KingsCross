...
class BBCEducationNews(config:NewsConfig)  extends News(config:NewsConfig) {
  //Emitter
  val emitter = context.actorOf(Props(classOf[NewsEmitter], config))
  // Collector
	val flowConfig = FlowControlConfig(name="bbcEducationCollector", actorType="com.reactor.kingscross.news.sources.BBCEducationNewsCollector")
	val collector = FlowControlFactory.flowControlledActorFor(context, flowConfig, CollectorArgs(config=config))

}


class BBCEducationNewsCollector(args:CollectorArgs) extends NewsCollector(args:CollectorArgs) {
