...
class JoystiqNews(config:NewsConfig)  extends News(config:NewsConfig) {
  //Emitter
  val emitter = context.actorOf(Props(classOf[NewsEmitter], config))
  // Collector
	val flowConfig = FlowControlConfig(name="joystiqCollector", actorType="com.reactor.kingscross.news.sources.JoystiqNewsCollector")
	val collector = FlowControlFactory.flowControlledActorFor(context, flowConfig, CollectorArgs(config=config))

}


class JoystiqNewsCollector(args:CollectorArgs) extends NewsCollector(args:CollectorArgs) {
  ...
  override def handleEvent(event:EmitEvent) {
    ...
    //	TODO: Load parameters from Mongo
      ...
      case Some(difbotAbstraction:Abstraction) =>
        ...
        story.full_text = getTextFromJsoup(story.link,new ExtractionRules("class", "post-body", List())) match {
