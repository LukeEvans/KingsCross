...
class UsaTodayHockeyNews(config:NewsConfig)  extends News(config:NewsConfig) {
  //Emitter
  val emitter = context.actorOf(Props(classOf[NewsEmitter], config))
  // Collector
	val flowConfig = FlowControlConfig(name="usaTodayHockeyCollector", actorType="com.reactor.kingscross.news.sources.UsaTodayHockeyNewsCollector")
	val collector = FlowControlFactory.flowControlledActorFor(context, flowConfig, CollectorArgs(config=config))

}


class UsaTodayHockeyNewsCollector(args:CollectorArgs) extends NewsCollector(args:CollectorArgs) {
  ...
  override def handleEvent(event:EmitEvent) {
    ...
    //	Fill out preliminary News Story fields
	  ...
	  story.source_id = "usatoday_hockey"
