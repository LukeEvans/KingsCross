...
class LottieDoofNews(config:NewsConfig)  extends News(config:NewsConfig) {
  //Emitter
  val emitter = context.actorOf(Props(classOf[NewsEmitter], config))
  // Collector
	val flowConfig = FlowControlConfig(name="lottieDoofCollector", actorType="com.reactor.kingscross.news.sources.LottieDoofNewsCollector")
	val collector = FlowControlFactory.flowControlledActorFor(context, flowConfig, CollectorArgs(config=config))

}


class LottieDoofNewsCollector(args:CollectorArgs) extends NewsCollector(args:CollectorArgs) {
