...
class NPRBusinessNews(config:NewsConfig)  extends News(config:NewsConfig) {
  //Emitter
  val emitter = context.actorOf(Props(classOf[NewsEmitter], config))
  // Collector
	val flowConfig = FlowControlConfig(name="nprBusinessCollector", actorType="com.reactor.kingscross.news.sources.NPRBusinessNewsCollector")
	val collector = FlowControlFactory.flowControlledActorFor(context, flowConfig, CollectorArgs(config=config))

}


class NPRBusinessNewsCollector(args:CollectorArgs) extends NewsCollector(args:CollectorArgs) {
