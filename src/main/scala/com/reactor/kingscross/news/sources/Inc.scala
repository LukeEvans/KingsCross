...
//================================================================================
// 	Inc.
//  Notes: - abstract with Difbot and Goose
//================================================================================

class IncNews(config:NewsConfig)  extends News(config:NewsConfig) {
  //Emitter
  val emitter = context.actorOf(Props(classOf[NewsEmitter], config))
  // Collector
	val flowConfig = FlowControlConfig(name="incCollector", actorType="com.reactor.kingscross.news.sources.IncNewsCollector")
	val collector = FlowControlFactory.flowControlledActorFor(context, flowConfig, CollectorArgs(config=config))

}


class IncNewsCollector(args:CollectorArgs) extends NewsCollector(args:CollectorArgs) {
  ...
  override def handleEvent(event:EmitEvent) {
    ...
    //	TODO: Load parameters from Mongo
	  ...
	  /*val difbotAbstraction:Option[Abstraction] = abstractWithDifbot(story.link)
    val gooseAbstraction:Option[Abstraction] = abstractWithGoose(story.link)

    difbotAbstraction match {
      case Some(difbot:Abstraction) =>
        gooseAbstraction match {
          case Some(goose:Abstraction) =>
            story.parseAbstraction(goose,difbot)
            // Get images from difbot
            //	Handle Images (custom to each news source)
            if (difbot.primary_images.size > 0) {
              story.image_links = difbot.primary_images
            }
            else {
              story.image_links = difbot.secondary_images
            }
          case None =>
            story.parseAbstraction(difbot)
            // Get images from difbot
            //	Handle Images (custom to each news source)
            if (difbot.primary_images.size > 0) {
              story.image_links = difbot.primary_images
      ...
      case None =>
        gooseAbstraction match {
          case Some(goose:Abstraction) =>
            story.parseAbstraction(goose)
            // Get images from goose
            //	Handle Images (custom to each news source)
            if (goose.primary_images.size > 0) {
              story.image_links = goose.primary_images
            }
            else {
              story.image_links = goose.secondary_images
            }
          case None =>
            println("COLLECTOR ERROR - Story creation failed at abstraction creation for "+story.source_id)
            complete()
            return
        }
    }*/
