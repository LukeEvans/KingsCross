...
class NPREntertainmentNews(config:NewsConfig)  extends News(config:NewsConfig) {
  //Emitter
  val emitter = context.actorOf(Props(classOf[NewsEmitter], config))
  // Collector
	val flowConfig = FlowControlConfig(name="nprEntertainmentCollector", actorType="com.reactor.kingscross.news.sources.NPREntertainmentNewsCollector")
	val collector = FlowControlFactory.flowControlledActorFor(context, flowConfig, CollectorArgs(config=config))

}


class NPREntertainmentNewsCollector(args:CollectorArgs) extends NewsCollector(args:CollectorArgs) {
  ...
  override def handleEvent(event:EmitEvent) {
    ...
    //	TODO: Load parameters from Mongo
	  story.ceiling_topic = "entertainment"
