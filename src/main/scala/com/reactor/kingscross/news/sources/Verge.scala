...
class VergeNews(config:NewsConfig)  extends News(config:NewsConfig) {
  //Emitter
  val emitter = context.actorOf(Props(classOf[NewsEmitter], config))
  // Collector
	val flowConfig = FlowControlConfig(name="vergeCollector", actorType="com.reactor.kingscross.news.sources.VergeNewsCollector")
	val collector = FlowControlFactory.flowControlledActorFor(context, flowConfig, CollectorArgs(config=config))

}


class VergeNewsCollector(args:CollectorArgs) extends NewsCollector(args:CollectorArgs) {
