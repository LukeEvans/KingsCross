...
class OnionNews(config:NewsConfig)  extends News(config:NewsConfig) {
  //Emitter
  val emitter = context.actorOf(Props(classOf[NewsEmitter], config))
  // Collector
	val flowConfig = FlowControlConfig(name="onionCollector", actorType="com.reactor.kingscross.news.sources.OnionNewsCollector")
	val collector = FlowControlFactory.flowControlledActorFor(context, flowConfig, CollectorArgs(config=config))

}


class OnionNewsCollector(args:CollectorArgs) extends NewsCollector(args:CollectorArgs) {
