...
class NPRHeadlineNews(config:NewsConfig) extends News(config:NewsConfig) {
   //Emitter
   val emitter = context.actorOf(Props(classOf[NewsEmitter], config))
   // Collector
   val flowConfig = FlowControlConfig(name="nprHeadlineCollector", actorType="com.reactor.kingscross.news.sources.NPRHeadlineNewsCollector")
   val collector = FlowControlFactory.flowControlledActorFor(context, flowConfig, CollectorArgs(config=config))

 }


class NPRHeadlineNewsCollector(args:CollectorArgs) extends NewsCollector(args:CollectorArgs) {
