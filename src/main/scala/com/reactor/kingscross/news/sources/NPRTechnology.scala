...
class NPRTechnologyNews(config:NewsConfig) extends News(config:NewsConfig) {
   //Emitter
   val emitter = context.actorOf(Props(classOf[NewsEmitter], config))
   // Collector
   val flowConfig = FlowControlConfig(name="nprTechnologyCollector", actorType="com.reactor.kingscross.news.sources.NPRTechnologyNewsCollector")
   val collector = FlowControlFactory.flowControlledActorFor(context, flowConfig, CollectorArgs(config=config))

 }


class NPRTechnologyNewsCollector(args:CollectorArgs) extends NewsCollector(args:CollectorArgs) {
