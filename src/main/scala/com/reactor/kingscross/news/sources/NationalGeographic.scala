...
class NationalGeographicNews(config:NewsConfig) extends News(config:NewsConfig) {
    //Emitter
    val emitter = context.actorOf(Props(classOf[NewsEmitter], config))
    // Collector
    val flowConfig = FlowControlConfig(name="nationalGeographicCollector", actorType="com.reactor.kingscross.news.sources.NationalGeographicNewsCollector")
    val collector = FlowControlFactory.flowControlledActorFor(context, flowConfig, CollectorArgs(config=config))

  }


class NationalGeographicNewsCollector(args:CollectorArgs) extends NewsCollector(args:CollectorArgs) {
