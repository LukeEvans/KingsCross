...
class AdventureBlogNews(config:NewsConfig)  extends News(config:NewsConfig) {
  ...
  // Collector
	val flowConfig = FlowControlConfig(name="adventureBlogCollector", actorType="com.reactor.kingscross.news.sources.AdventureBlogNewsCollector")
	val collector = FlowControlFactory.flowControlledActorFor(context, flowConfig, CollectorArgs(config=config))

}


class AdventureBlogNewsCollector(args:CollectorArgs) extends NewsCollector(args:CollectorArgs) {
