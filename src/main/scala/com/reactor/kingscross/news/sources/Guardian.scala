...
class GuardianNews(config:NewsConfig)  extends News(config:NewsConfig) {
  //Emitter
  val emitter = context.actorOf(Props(classOf[NewsEmitter], config))
  // Collector
	val flowConfig = FlowControlConfig(name="guardianCollector", actorType="com.reactor.kingscross.news.sources.GuardianNewsCollector")
	val collector = FlowControlFactory.flowControlledActorFor(context, flowConfig, CollectorArgs(config=config))

}


class GuardianNewsCollector(args:CollectorArgs) extends NewsCollector(args:CollectorArgs) {
