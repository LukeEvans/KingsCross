...
class ReutersTopNews(config:NewsConfig)  extends News(config:NewsConfig) {
  //Emitter
  val emitter = context.actorOf(Props(classOf[NewsEmitter], config))
  // Collector
	val flowConfig = FlowControlConfig(name="reutersTopCollector", actorType="com.reactor.kingscross.news.sources.ReutersTopNewsCollector")
	val collector = FlowControlFactory.flowControlledActorFor(context, flowConfig, CollectorArgs(config=config))

}


class ReutersTopNewsCollector(args:CollectorArgs) extends NewsCollector(args:CollectorArgs) {
