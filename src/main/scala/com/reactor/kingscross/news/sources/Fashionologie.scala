...
class FashionologieNews(config:NewsConfig)  extends News(config:NewsConfig) {
  //Emitter
  val emitter = context.actorOf(Props(classOf[NewsEmitter], config))
  // Collector
	val flowConfig = FlowControlConfig(name="fashionologieCollector", actorType="com.reactor.kingscross.news.sources.FashionologieNewsCollector")
	val collector = FlowControlFactory.flowControlledActorFor(context, flowConfig, CollectorArgs(config=config))

}


class FashionologieNewsCollector(args:CollectorArgs) extends NewsCollector(args:CollectorArgs) {
